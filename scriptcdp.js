...
let omcIcon = L.icon({
    iconUrl: 'assets/om.png', // Replace with the path to your O&M center icon
...
function drawRoute(encodedGeometry) {
    ...
    // Add traffic spots along the route
    addTrafficSpots(decodedCoords);
}

// Function to add traffic spots along the route
function addTrafficSpots(routeCoords) {
    const trafficSpots = 3; // Number of traffic spots to add
    for (let i = 0; i < trafficSpots; i++) {
        // Randomly select a point along the route
        const index = Math.floor(Math.random() * routeCoords.length);
        const [lat, lon] = routeCoords[index];

        // Create a traffic spot marker
        L.circleMarker([lat, lon], { color: 'red', radius: 8, fillOpacity: 0.6 })
            .addTo(map)
            .bindPopup("Traffic Spot: Expect delays here!");
    }
}
...
function resetMap() {
    ...
    // Remove traffic spots from the map
    clearTrafficSpots();
}

// Function to clear traffic spots
function clearTrafficSpots() {
    // Loop through all traffic spots and remove them from the map
...
function mic(id) {
    console.log(`Microphone ${id} toggled.`);
